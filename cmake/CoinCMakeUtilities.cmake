...
function(coin_add_pkg_config_lib_dependencies _output _input)
  foreach(_lib ${_input})
    ...
    if(_lib MATCHES "^-")
      list(APPEND ${_output} "${_lib}")
    else()
      list(APPEND ${_output} "-l${_lib}")
    endif()
  endforeach()
...
# compile definitions, from _tgt to pkg-config settings
macro(coin_get_pkg_config_target_properties _tgt)
  if(TARGET ${_tgt})
    ...
    if (NOT ${_tgt_type} STREQUAL "INTERFACE_LIBRARY")
      ...
      if(_has_prop)
        ...
        coin_add_pkg_config_lib_dependencies(LIB_DEPENDENCIES ${_imp_loc})
      else()
      ...
      if(_has_prop)
        ...
        coin_add_pkg_config_lib_dependencies(LIB_DEPENDENCIES ${_imp_lib})
      else()
    ...
    if(_has_prop)
      ...
      list(APPEND PKG_CONFIG_CPPFLAGS "$<$<BOOL:${_compile_defs}>:-D$<JOIN:${_compile_defs}, -D>>")
    endif()
    ...
    if(_has_prop)
      ...
      list(APPEND PKG_CONFIG_CPPFLAGS "$<$<BOOL:${_compile_opts}>:$<JOIN:${_compile_opts}, >>")
    endif()
  endif()
endmacro()
