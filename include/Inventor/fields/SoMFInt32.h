...
#ifndef __SOMFINT32_H__
#define __SOMFINT32_H__
...
public:
  ...
  const int32_t * getValues(const int start) const
    { this->evaluate(); return (const int32_t *)(this->values + start); }
  int find(int32_t value, SbBool addIfNotFound = FALSE);
  void setValues(const int start, const int num, const int32_t * values);
  ...
  SbBool operator == (const SoMFInt32 & field) const;
