...
#ifndef __SOMFSHORT_H__
#define __SOMFSHORT_H__
...
public:
  ...
  const SoMFShort & operator = (const SoMFShort & field);
  virtual SbBool isSame(const SoField & field) const;
public:
  SoMFShort(void);
  virtual ~SoMFShort(void);
private:
...
public:
  ...
  const short * getValues(const int start) const
    { this->evaluate(); return (const short *)(this->values + start); }
  int find(short value, SbBool addIfNotFound = FALSE);
  void setValues(const int start, const int num, const short * values);
  void set1Value(const int idx, short value);
  void setValue(short value);
  /*! Make field contain a just the single value \a val. */
  short operator = (short val)
    { this->setValue(val); return val; }
  SbBool operator == (const SoMFShort & field) const;
