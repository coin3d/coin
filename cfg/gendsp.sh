...
#
# The level of sed usage with ad-hoc quoting issue fixes in this script has
# now officially reached the point of madness.  If developing this script
# further, a complete rewrite to a more sensible language should be
...
if test x"$1" = x"--register-public-header"; then
  ...
  * )
  ...
  installheadersfile=`echo $studiofile | sed -e 's,\([^\\/0-9]*\)[0-9][^\\/]*\.dsp,install\1headers.bat,'`
  ...
  echo "copy $relheader %COINDIR%\\$finalheader" >>"$installheadersfile"
  exit 0
fi
...
if test -f "$studiofile"; then :; else
  ...
  echo "PostBuild_Cmds=install${library}.bat" >>"$studiofile"
...
*.so.* )
  ...
  builddirfixed=`echo "$builddir" | sed -e 's/\\\\/\\\\\\\\/g'`
  ...
  sourcedirfixed=`echo "$sourcedir" | sed -e 's/\\\\/\\\\\\\\/g'`
  ...
  ( cd $builddir_unix; make INSTALL_HEADER="$0 --register-public-header $sourcedirfixed $builddirfixed $studiofile" DESTDIR=/tmp/coin-dsp install-data )
  rm -rf /tmp/coin-dsp
  echo '# End Group' >>"$studiofile"
