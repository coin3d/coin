...
SbDPRotation::setValue(const SbVec3d & rotateFrom, const SbVec3d & rotateTo)
{
  ...
  const double crosslen = crossvec.length();
  ...
  if (crosslen == 0.0f) { // Parallel vectors
    ...
    // of each other.
      ...
      if(t.length() == 0.0f) t = from.cross(SbVec3d(0.0f, 1.0f, 0.0f));
 ...
 */
SbBool
SbDPRotation::equals(const SbDPRotation & r, const double tolerance) const
{
